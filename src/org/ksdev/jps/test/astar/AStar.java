package org.ksdev.jps.test.astar;
...
public class AStar {
	...
	// 这些数据列表都以节点ID为索引顺序存储。
	private List<Hex>   m_List;					//节点x,y坐标列表
	...
	private Map<Hex, Node> m_noteMap;				//节点(数组)地图,根据节点坐标记录节点开启关闭状态和ID
	...
	/**
	 * Constructor
	 *  @param data    地图模型，实现 IMapTileModel 接口
	 * @param p_maxTry            最大寻路步数，限制超时返回
	 */
	public AStar(Map<Long, Byte> data, int p_maxTry)
	{
		this.data 		= data;
	...
	public List<Hex> find(Hex start, Hex end)
	{
		...
		this.openNote(start, 0, 0, 0);			// 起点放到开启列表中
		
		int currId;
		Hex currPoint;
		int preId;
		List<Hex> aroundNotes;
		...
		while (this.m_openCount > 0)
		{
			...
			Hex point = this.m_List.get(currId);
			currPoint = point;

			//如果终点被放入关闭列表寻路结束，返回路径
			if (point.equals(end))
			{
				return this.getPath(point, currId);
			}
			//获取周围节点，排除不可通过和已在关闭列表中的
			aroundNotes = this.getArounds(currPoint);
			
			// 前面一个点
			preId    = this.m_fatherList.get(currId);
			Hex preHex = this.m_List.get(preId);

			//对于周围的每一个节点
			for(Hex note : aroundNotes)
			{
				//计算F和G值
...
//					}
				...
				if(preHex!=null){
					if(!(currPoint.x <<1 == preHex.x +note.x &&currPoint.z <<1 == preHex.z +note.z &&currPoint.y <<1 == preHex.y +note.y)){
...
//					cost = this.m_movementCostList[currId] + ((note[0] == currNoteX || note[1] == currNoteY) ? COST_STRAIGHT : COST_DIAGONAL);
				...
				score = cost + ( note.distance(end) ) * COST_STRAIGHT;

//					score = cost +  Math.abs(p_endX - note[0]) * this.COST_DIAGONAL +
//									Math.abs(p_endY - note[1]) * this.COST_VERTICAL;		// 这句会认人走得不自然		

				//Hex point2 = new Hex(note[0], note[1]);
				if (this.isOpen(note)) //如果节点已在播放列表中
				{
					Node node = this.m_noteMap.get( note );
	 ...
	 * 将节点加入开放列表
	 *
	 * @param point		节点在地图中的x坐标
	 * @param point		节点在地图中的y坐标
	 * @param p_score	节点的路径评分
	 * @param p_cost	起始点到节点的移动成本
	 * @param p_fatherId	父节点
	 */
	private void openNote(Hex point, int p_score, int p_cost, int p_fatherId)
	{
		this.m_openCount++;
		this.m_openId++;
	...
	{
		this.m_openCount--;
		Hex point = this.m_List.get(p_id);
	 ...
	 * @return
	 */
	private boolean isOpen(Hex point)
	{
	 ...
	 * 判断某节点是否在关闭列表中
	 */
	private boolean isClosed(Hex point)
	{
		if (this.m_noteMap.get( point ) == null){
			return false;
		}
	 ...
	 */
	private List<Hex> getArounds(Hex p)
	{
		List<Hex> list = new ArrayList<>(6);

		//y&1 y是偶数是0，不属于红色方块某个，x左边-1.奇数是1 意思是，这个节点属于红色方块，说明x坐标不变

		for (int direct = 0; direct < 6; direct++) {
			Hex checkHex = p.diagonalNeighbor(direct);
			boolean canRightBottom = isBalk || isPass(checkHex);
			if (canRightBottom && !this.isClosed(checkHex))
			{
				list.add(checkHex);
			}
		}
...
//		}
	 ...
	 * @return 			路径坐标(Hex)数组
	 */
	private List<Hex> getPath(Hex start, int p_id)
	{
	...
	{
		this.m_openList 		= new ArrayList<>(20);
		this.m_List 			= new ArrayList<>(20);
		this.m_pathScoreList 	= new ArrayList<>(20);
		this.m_movementCostList = new ArrayList<>(20);
		this.m_fatherList 		= new ArrayList<>(20);
		this.m_noteMap 			= new HashMap<>(20);
	}
	 ...
	 */
	public int isBlock(Hex point)
	{
		long index = Hex.tolong(point);
		Byte mask = this.data.get(index);
		if(mask==null){
			return -1;
		}
		return mask;
	 ...
	 * @return
	 */
	public boolean isPass(Hex point){
		int mask = isBlock(point);
